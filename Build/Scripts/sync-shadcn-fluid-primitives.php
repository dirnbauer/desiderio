...
{
    return sprintf(
        "<f:comment>\n  Generated by %s.\n  shadcn preset: %s | style: %s\n  Source: https://ui.shadcn.com/r/styles/%s/{component}.json\n  Control shape is tokenized: corner radius follows the shadcn --radius scale and form-control height/text/padding follow the --d-control-* tokens, so shape switches per preset.\n</f:comment>\n",
        $scriptRelativePath,
        $preset,
        $style,
        $style
...
{
    $recipes['input'] = applyShapeTokens($recipes['input'], 'control', true);
    $recipes['textarea'] = applyShapeTokens($recipes['textarea'], 'control', true);
    $recipes['select']['trigger'] = applyShapeTokens($recipes['select']['trigger'], 'control', true);

    $recipes['button']['base'] = applyShapeTokens($recipes['button']['base'], 'control', true);
    foreach ($recipes['button']['sizes'] as $size => $value) {
        $recipes['button']['sizes'][$size] = applyShapeTokens($value, 'control', true);
    }

    $recipes['badge']['base'] = applyShapeTokens($recipes['badge']['base'], 'badge', false);

    foreach (['root', 'header', 'title', 'description', 'content', 'footer'] as $slot) {
        $recipes['card'][$slot] = applyShapeTokens($recipes['card'][$slot], 'card', false);
    }

    $recipes['checkbox']['root'] = applyShapeTokens($recipes['checkbox']['root'], 'checkbox', false);

    $recipes['tabs']['listBase'] = applyShapeTokens($recipes['tabs']['listBase'], 'control', false);
    foreach ($recipes['tabs']['listVariants'] as $variant => $value) {
        $recipes['tabs']['listVariants'][$variant] = applyShapeTokens($value, 'control', false);
    }
    $recipes['tabs']['trigger'] = applyShapeTokens($recipes['tabs']['trigger'], 'control', false);
 ...
 */
function applyShapeTokens(string $class, string $radiusKind, bool $density): string
{
    $canonicalRadius = [
        'control' => 'rounded-md',
        'card' => 'rounded-xl',
