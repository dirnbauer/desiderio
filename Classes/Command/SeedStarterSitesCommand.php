...
final class SeedStarterSitesCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach (array_values($selectedPresets) as $index => $starter) {
            ...
            foreach (array_values($starter['subpages']) as $pageIndex => $page) {
                $childSlug = rtrim($starter['rootSlug'], '/') . '/' . ltrim($page['slug'], '/');
        ...
        $io->success(sprintf(
            'Created %d and updated %d starter pages below page uid %d. Inserted %d Desiderio content elements for %d starter site%s.',
            $createdPages,
            $updatedPages,
            $parentPid,
    ...
    private function updateStarterPage(
        ...
        $this->connectionPool->getConnectionForTable('pages')->update(
            ...
            $this->filterRow([
                ...
                'backend_layout_next_level' => 'DesiderioContentpage',
                'hidden' => 0,
                'sorting' => $sorting,
                'tstamp' => $now,
            ], $columns),
     ...
     */
    private function seedPageContent(int $pageUid, array $contentBlocks, int $now, array $contentColumns): int
    {
        $this->markExistingStarterContentAsDeleted($pageUid, $now);
        $created = 0;
    ...
    private function markExistingStarterContentAsDeleted(int $pageUid, int $now): void
    {
