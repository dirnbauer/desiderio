...
- Ten selectable **house presets** (Aurora, Marine, Forest, Ember, Bloom,
  ...
  inherits the authentic neutral base and overrides only its accent colour,
  radius, fonts, icon library, and (for Citrus) compact density — so editors
  can switch the whole site theme from the site configuration with no rebuild.
